// compile with
// c++ -o sp2 setsP2.cpp -std=c++14 -fopenmp
// run with
// ./sp2
// compile and run with
// c++ -o sp2 setsP2.cpp -std=c++14 -fopenmp && ./sp2
...
public:
    ...
    void deleteElements(const std::vector<dataType> & );
...
SetParallel::SetParallel(const int& desiredNumThreads, //won't need later
    ...
    dataType intervalSize=(predictionMax-predictionMin) / ( static_cast<dataType>(numTrees-2) );

    for(int i=0;i<numTrees-2;++i){
        rightEndPoints[i]=predictionMin + i*intervalSize;
    }
    rightEndPoints[numTrees-2]=predictionMax;
    std::cout<<std::endl;
}
...
void SetParallel::deleteElements(const std::vector<dataType> & vectToDelete ){
  int numberOfThreads;
  #pragma omp parallel
  {
    if(omp_get_thread_num()==0){
...
int main(){
  ...
  firstTest.deleteElements(dataToDelete);
