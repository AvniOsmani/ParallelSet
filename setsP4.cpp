// compile with
// c++ -o sp4 setsP4.cpp -std=c++14 -fopenmp
// run with
// ./sp4
// compile and run with
// c++ -o sp4 setsP4.cpp -std=c++14 -fopenmp && ./sp4
...
int main(){
  int numberOfTrees;
  /*dataType minPrediction, maxPrediction;
  std::cout<<"Enter the following" <<std::endl;
  std::cout<<"number of trees: ";
  std::cin>>numberOfTrees;
  std::cout<<"minimum prediction: ";
  std::cin>>minPrediction;
  std::cout<<"maximum prediction: ";
  std::cin>>maxPrediction;*/

  SetParallel firstTest(4, 6, -1000, 1000);
  ...
  std::vector<dataType> dataToCheck;
  std::cout<<"How many numbers do you want to check? ";
  int totalToCheck;
  std::cin>>totalToCheck;
  dataToCheck.resize(totalToCheck);
  std::cout<<"give me " <<totalToCheck <<" numbers: ";
  for(int i=0; i<totalToCheck; ++i){
    std::cin>>dataToCheck[i];
  }
  std::vector<int> results;
  firstTest.checkElements(dataToCheck, results);
  ...
  /*std::vector<dataType> dataToDelete;
  std::cout<<"how many numbers do you want to delete? ";
  int totalToDelete;
  std::cin>>totalToDelete;
  dataToDelete.resize(totalToDelete);
  std::cout<<"give me " <<totalToDelete <<" numbers: ";
  for(int i=0; i<totalToDelete; ++i){
    std::cin>>dataToDelete[i];
  }
  firstTest.eraseParallel(dataToDelete);
