// compile with
// c++ -o i2spTemplate4 idea2_setsP_template4.cpp -std=c++14 -fopenmp
// run with
// ./i2spTemplate4
// compile and run with
// c++ -o i2spTemplate4 idea2_setsP_template4.cpp -std=c++14 -fno-elide-constructors -fopenmp && ./i2spTemplate4
...
private:
    ...
    void rebalanceTrees();
    void insertParallelNoRebalance(const std::vector<T> &);
public:
    SetParallel();
    ...
    T getRightEndPoint(const int &) const;
    virtual ~SetParallel();
};

template <class T>
void SetParallel<T>::rebalanceTrees(){
  std::vector<T> rebalanceData;
  rebalanceData.resize(sizeP());
  /*int counter = 0;
  typename std::set<T>::iterator iter;
  for(int i=0; i<allData.size(); ++i){
    for(iter=allData[i].begin(); iter!=allData[i].end(); ++iter){
      rebalanceData[counter] = (*iter);
      ++counter;
    }
  }*/
  ...
  #pragma omp parallel num_threads(numThreads)
  {
    ...
    for(iter=allData[myID].begin(); iter!=allData[myID].end(); ++iter){
      rebalanceData[position] = (*iter);
      ++position;
    }
  }
  #pragma omp barrier

  if(rebalanceData.size() == numThreads-1){
    for(int i=0; i<rightEndPoints.size(); ++i){
      rightEndPoints[i]=rebalanceData[i];
    }
  }
  else{
    int intervalSize = rebalanceData.size() / numThreads;
    for(int i=0; i<rightEndPoints.size(); ++i){
      rightEndPoints[i]=rebalanceData[(i+1)*intervalSize];
    }
  }

  clearP();
  std::cout<<"is set empty: " <<emptyP() <<std::endl;
  insertParallelNoRebalance(rebalanceData);
}
...
template <class T>
void SetParallel<T>::insertParallelNoRebalance(const std::vector<T> & _vectToInsert ){
  #pragma omp parallel num_threads(numThreads)
  {
    int myID=omp_get_thread_num();
    for(int vectToInsertPosition=0; vectToInsertPosition<_vectToInsert.size(); ++vectToInsertPosition){
...
template <class T>
T SetParallel<T>::getRightEndPoint(const int & _i) const{
...
int main(){
  ...
  std::cout<<"the first right end point is " <<stringTest.getRightEndPoint(0) <<std::endl;
  std::cout<<"the second right end point is " <<stringTest.getRightEndPoint(1) <<std::endl;
  std::cout<<"the third right end point is " <<stringTest.getRightEndPoint(2) <<std::endl;
  ...
  /*std::vector<std::string> newrep;
  newrep.resize(3);
  newrep[0]="J";
  newrep[1]="Y";
  newrep[2]="f";

  stringTest.setRightEndPoints(newrep);
  ...
  moreData[4]="arm";
  moreData[5]="ball";
  moreData[6]="chocolate";
  moreData[7]="alligator";
  moreData[8]="brooklyn";

  stringTest.insertParallel(moreData);
  stringTest.printAllTrees();
  std::cout<<"the first right end point is " <<stringTest.getRightEndPoint(0) <<std::endl;
  std::cout<<"the second right end point is " <<stringTest.getRightEndPoint(1) <<std::endl;
  std::cout<<"the third right end point is " <<stringTest.getRightEndPoint(2) <<std::endl;
